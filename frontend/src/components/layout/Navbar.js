...
function CustomNavbar() {
    ...
    const [isAuthenticated, setIsAuthenticated] = useState(!!(localStorage.getItem('token') || localStorage.getItem('access_token')));
    ...
    useEffect(() => {
        // Загружаем данные пользователя (JWT или DRF token)
        const token = localStorage.getItem('token');
        const accessToken = localStorage.getItem('access_token');
        const userData = localStorage.getItem('user');
        if ((token || accessToken) && userData) {
            try {
    ...
    const handleLogout = async () => {
        ...
        // Очищаем localStorage (и DRF token, и JWT)
